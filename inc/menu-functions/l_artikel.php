...
function l_artikel() {
    ...
    $qry = $sql->select("SELECT `id`,`titel`,`text`,`autor`,`datum`,`kat`,`public` "
            . "FROM `{prefix_artikel}` "
            . "WHERE `public` = 1 "
            . "ORDER BY `id` DESC LIMIT ".config('m_lartikel').";");
    ...
    if($sql->rowCount()) {
        foreach($qry as $get) {
            ...
            $info = !config('allowhover') == 1 ? '' : 'onmouseover="DZCP.showInfo(\''.jsconvert(re($get['titel'])).'\', \''._datum.';'.
            ...
            $l_articles .= show("menu/last_artikel", array("id" => $get['id'],
                                                           "titel" => cut(re($get['titel']),config('l_lartikel')),
