...
if (!isset($_SESSION['id']) || empty($_SESSION['id']) || !admin_perms($_SESSION['id'])) {
    ...
    if (isset($_GET['admin']) && file_exists(basePath . '/admin/menu/' . secure_global_imput($_GET['admin']).'.php') &&
        ...
        if (permission($rights) && !$oa && !$ora)
            $permission = true;
        if ($oa && !$ora && $chkMe == 4)
            $permission = true;
        if ($ora && $chkMe == 4 && rootAdmin())
            $permission = true;
    ...
    if (empty($show)) {
        ...
        if (allow_url_fopen_support()) {
            if (admin_view_dzcp_news) {
                ...
                $dzcp_news_db = array();
                if (file_exists(basePath . '/admin/admin_news.json')) {
                    $dzcp_news_db = json_decode(file_get_contents(basePath . '/admin/admin_news.json'), true);
                    if (isset($_POST['what']) && $_POST['what'] == 'news') {
                        ...
                        file_put_contents(basePath . '/admin/admin_news.json', json_encode($dzcp_news_db));
                    }
                } else {
