...
switch ($do) {
    ...
    break;
    case 'delete':
        $get = $sql->fetch("SELECT id,sid FROM `{prefix_forumsubkats}` WHERE sid = '".intval($_GET['id'])."'");
        if($sql->rowCount()) {
            $sql->delete("DELETE FROM `{prefix_forumkats}` WHERE `id` = ?;",array($get['sid']));
            $sql->delete("DELETE FROM `{prefix_forumthreads}` WHERE `kid` = ?;",array($get['sid']));
            $sql->delete("DELETE FROM `{prefix_forumposts}` WHERE `kid` = ?;",array($get['sid']));
            $sql->delete("DELETE FROM `{prefix_forumsubkats}` WHERE `sid` = ?;",array($get['sid']));
    ...
    case 'editskat':
        if(empty($_POST['skat'])) { 
            ...
            $sql->update("UPDATE `{prefix_forumsubkats}` SET "
        ...
        if(isset($_GET['show']) && strtolower($_GET['show']) == "subkats") {
            ...
            foreach($qry as $get) {
                ...
                $kats .= show($dir."/forum_show_kats", array("class" => $class,
                                                             "kat" => $kat,
                                                             "status" => $status,
                                                             "skats" => cnt('{prefix_forumsubkats}', " WHERE sid = ?","id",array(intval($get['id']))),
                                                             "edit" => $edit,
                                                             "delete" => $delete));
            }
